...
router.get('/organizaciones', async (req, res) => {
    try {
        ...
        return res.json({
            status: 200,
            success: true,
            details: 'Organizaciones encontradas correctamente',
            organizations
        });
    } catch (error) {
    ...
    try {
        ...
        if (!organizations || organizations.length === 0) {
            return res.status(404).json({
                success: false,
                details: 'No se encontraron organizaciones para este usuario'
            });
        }
...
router.get('/organizaciones/byId/:id', async (req, res) => {
    try {
        ...
        return res.json({
            status: 200,
            success: true,
            details: 'Organizacion encontrada correctamente',
...
router.put('/organizaciones/:id', async (req, res) => {
    try {
        ...
        if (members) {
            ...
            for (const [activityIndex, activity] of updatedOrganizacion.activities.entries()) {
                adminMembers.forEach(adminMember => {
                    if (!activity.members.some(member => member._id.toString() === adminMember._id.toString())) {
                ...
                for (const [groupIndex, group ] of activity.groups.entries()) {
                    for (const adminMember of adminMembers) {
                        const existingMemberIndex = group.members.findIndex(member => member._id.toString() === adminMember._id.toString());
                        if (existingMemberIndex === -1) {
                            console.log("Pushing admin member: ", adminMember);
                            updatedOrganizacion.activities[activityIndex].groups[groupIndex].members.push(adminMember);
                        } else {
                            console.log("Changing role of admin member: ", adminMember)
                            updatedOrganizacion.activities[activityIndex].groups[groupIndex].members[existingMemberIndex].role = adminMember.role;
                        }
                    }
                }
            }
        ...
        await organizationModel.findByIdAndUpdate(updatedOrganizacion._id, updatedOrganizacion);

        return res.json({
            status: 200,
            success: true,
            details: 'Organización actualizada correctamente',
            activity: updatedOrganizacion
        });
    } catch (error) {
